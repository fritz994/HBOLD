...
def getExtById(id):
    client = pm.MongoClient()
    return [a for a in client.lodex.ext.find({'id':id})]
...
def startTestNew(endpoint):
    client = pm.MongoClient()
    currRun = client.lodex.runInfo.find_one({'id': endpoint['_id']} if '_id' in endpoint else None, sort=[("run", -1)])
    currRunNumber = 1
    if currRun is not None:
        currRunNumber = currRun['run']+1

    a = {'id': endpoint['_id'] if '_id' in endpoint else None,
           'run': currRunNumber,
            'datasets': endpoint['datasets'] if 'datasets' in endpoint else None,
            'url': endpoint['url'] if 'url' in endpoint else None,
...
def updateLodexDatasets(end, datasets_list):
    client = pm.MongoClient()
    client.lodex.endpoints.update({'_id':end['_id']}, {'url':end['url'], 'name':end['name'], 'datasets':datasets_list})
...
def getEndopointByUrl(url):
    client = pm.MongoClient()
    db=client.lodex
    return db.endpoints.find_one({'url':url})

def getAllEndopoinLodex1():
    client = pm.MongoClient()
    db=client.lodex
    for e in db.endpoints1.find({}).sort("_id", 1):
        yield e
...
def deleteExtById(id):
    client = pm.MongoClient()
    client.lodex.ext.remove({'id': id})
